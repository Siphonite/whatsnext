...
// auto generated by anchor (from IDL)
...
impl SolanaClient {
    ...
    // -------------------------------------------------------
    // Initialize Treasury (Called at Backend Startup)
    // -------------------------------------------------------

    pub fn initialize_treasury_if_needed(&self) -> Result<()> {
        let program = self.program();
        let (treasury_pda, bump) = self.derive_treasury_pda();

        // check if account exists
        if program.rpc().get_account(&treasury_pda).is_ok() {
            println!("[TREASURY] Already exists at {}", treasury_pda);
            return Ok(());
        }

        println!("[TREASURY] Creating treasury PDA {}", treasury_pda);

        let accounts = accounts_mod::InitializeTreasury {
            treasury: treasury_pda,
            authority: self.payer.pubkey(),
            system_program: Self::system_program_id(),
        };

        let args = instruction_mod::InitializeTreasury {};
    ...
    pub fn settle_market_and_send(
        ...
        let args = instruction_mod::SettleMarket { close_price };
        ...
        let sig = program
            ...
            .signer(&*self.payer)
            .send()
            .map_err(|e| anyhow!("Failed to send settle tx: {}", e))?;
